...
export default function RoofBarsConfigurator() {
  ...
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      if (field === "brand") {
        ...
        if (value) {
          fetchOptions("/api/roof-bars/get-models", { brand: value }, setModelOptions);
        }
        return;
      }
    ...
    async (e: React.FormEvent) => {
      ...
      try {
        const response = await fetch(`/api/roof-bars/search?${qs}`);
  ...
  const resetAll = useCallback(() => {
    setForm({ brand: "", model: "", year: "", type: "", manufacturer: "" });
    setModelOptions({ data: [] });
    setYearOptions({ data: [] });
    setTypeOptions({ data: [] });
    setManufacturerOptions({ data: [] });
